...
# GEMINI.md: Antigravity 로컬 규칙 인덱스
...
2.  **Maintain Order**: 모든 산출물은 지정된 폴더(`.agent/tasks/...`)에 체계적으로 관리되어야 합니다.
